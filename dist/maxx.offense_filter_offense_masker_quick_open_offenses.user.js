...
"Process CommandLine" AS 'Command Line'`,
    ...
    function openAqlGUI(doc) {
      ...
      async function runSyntaxCheck(doc2) {
        const resEl = doc2.querySelector("#mx-syntax-result");
        ...
        if (!csrfToken) {
          resEl.textContent = "⏳ Đang tìm Token... Hãy thử tắt bảng này đi, thao tác nhẹ trên giao diện QRadar rồi thử lại!";
          resEl.style.color = "#f59e0b";
          return;
        }
        resEl.textContent = "⏳ Đang kiểm tra...";
        resEl.style.color = "#3b82f6";
        let sel = doc2.querySelector("#mx-query").value.trim().replace(/;$/, "");
        if (sel.toUpperCase().startsWith("SELECT ")) sel = sel.substring(7).trim();
        const useDef = doc2.querySelector("#mx-use-defaults").checked;
        const gOpts = useDef ? loadGlobalDefaults() : { select: "", where: "" };
        if (useDef && gOpts.select) {
          sel = sel ? `${sel}, ${gOpts.select}` : gOpts.select;
        }
        const customWh = doc2.querySelector("#mx-where").value.trim();
        const enGrp = doc2.querySelector("#mx-en-group").checked;
        const grp = doc2.querySelector("#mx-group").value.trim();
        const enOrd = doc2.querySelector("#mx-en-order").checked;
        const ord = doc2.querySelector("#mx-order").value.trim();
...
Bỏ trống nếu không cần thêm điều kiện.">${mapping.whereQuery || ""}</textarea>
        ...
        viewEl.querySelector("#mx-use-defaults").onchange = (e) => {
          ...
          viewEl.querySelector("#mx-preview-select-suffix").innerHTML = isChecked && currentOpts.select ? `, <span class="mx-aql-ide-fn">${currentOpts.select}</span><br><span class="mx-aql-ide-kw">FROM</span> events` : `<br><span class="mx-aql-ide-kw">FROM</span> events`;
          viewEl.querySelector("#mx-preview-where-prefix").innerHTML = isChecked && currentOpts.where ? `<span class="mx-aql-ide-kw">AND</span> <span class="mx-aql-ide-fn">(${currentOpts.where})</span>` : ``;
        };
